...
final class PlaidSyncServiceTests: XCTestCase {
    func testSyncAppliesPlaidTransactionsSyncChangesToDatabase() async throws {
        ...
        XCTAssertEqual(snapshot.transactions.first { $0.id == "txn-payroll" }?.categoryID, "default-income")
        XCTAssertEqual(snapshot.transactions.first { $0.id == "txn-payroll" }?.categoryAssignmentSource, .plaid)
        XCTAssertEqual(snapshot.transactions.first { $0.id == "txn-rent" }?.categoryID, "default-housing")
        XCTAssertEqual(snapshot.transactions.first { $0.id == "txn-rent" }?.categoryAssignmentSource, .plaid)
